<script lang="ts">
	...
	import {
		getLaunchesThisMonth,
		getTotalLaunches,
		getRecentLaunches,
		getDefaultFlavorStats,
	} from '$lib/systems/FlavorMechanics';

	let { color = '#FF4444' }: { color?: string } = $props();

	let stats = $derived($gameState.flavorStats ?? getDefaultFlavorStats());
	let launchesThisMonth = $derived(getLaunchesThisMonth(stats));
	let totalLaunches = $derived(getTotalLaunches(stats));
	let recentLaunches = $derived(getRecentLaunches(stats, 5));
	...
	<div class="p-3">
		...
		<div class="grid grid-cols-2 gap-2 mb-3">
			<div class="bg-black/20 rounded-lg p-2 text-center">
				<div class="text-[10px] text-text-muted uppercase tracking-wider">This Month</div>
				<div class="text-lg font-bold tabular-nums" style="color: {color};">
					{launchesThisMonth}
				</div>
				<div class="text-[9px] text-text-muted">launches</div>
			</div>
			<div class="bg-black/20 rounded-lg p-2 text-center">
				...
				<div class="text-[9px] text-text-muted">total launches</div>
			</div>
		</div>
		...
		{#if recentLaunches.length > 0}
			<div class="space-y-1">
				...
				{#each recentLaunches as entry (entry.ts)}
		...
		{:else}
			<div class="text-[11px] text-text-muted text-center py-2 italic">
				No launches yet. Start producing rockets!
			</div>
		{/if}
	</div>
</div>
